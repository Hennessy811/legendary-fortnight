...
class Model {
  NEW_OD_OUTPUT_TENSORS: string[]
  model: tf.GraphModel
  input_size: any
  is_new_od_model: boolean
  is_rgb_input: any

  constructor() {
    this.NEW_OD_OUTPUT_TENSORS = [
      "detected_boxes",
      "detected_scores",
      "detected_classes",
    ]
  ...
  async loadModelAsync(modelUrl, options = null) {
    ...
    this.is_rgb_input =
      (this.model.metadata &&
        this.model.metadata["Image.BitmapPixelFormat"] == "Rgb8") ||
      this.is_new_od_model
  }
  ...
  async executeAsync(pixels) {
    const inputs = tf.tidy(() => {
      return pixels instanceof tf.Tensor
        ? pixels
        : this._preprocess(tf.node.decodeImage(pixels, 3))
    })
    const outputs = await this.model.executeAsync(
      inputs,
      this.is_new_od_model ? this.NEW_OD_OUTPUT_TENSORS : null
    )
    tf.dispose(inputs)
    const arrays = await (!Array.isArray(outputs)
      ? outputs.array()
      : Promise.all(outputs.map((t) => t.array())))
    tf.dispose(outputs)
    return this._postprocess(arrays)
  }
  _preprocess(arg0: tf.Tensor3D | tf.Tensor4D): any {
...
export class ObjectDetectionModel extends Model {
  ...
  _preprocess(image) {
    const rgb_image = tf.image.resizeBilinear(image.expandDims().toFloat(), [
      this.input_size,
      this.input_size,
    ])
  ...
  async _postprocess(outputs) {
        ...
        for (var grid_x = 0; grid_x < width; grid_x++) {
          ...
          for (var i = 0; i < num_anchor; i++) {
            ...
            let objectness = tf.scalar(
              this._logistic(outputs[0][grid_y][grid_x][offset++])
            )
    ...
    const tensor_results = [
      // @ts-ignore
      boxes.gather(selected_indices),
      // @ts-ignore
      scores.gather(selected_indices),
      // @ts-ignore
      classes.gather(selected_indices),
    ]
    const results = [
      await tensor_results[0].array(),
      await tensor_results[1].array(),
      await tensor_results[2].array(),
    ]
...
export class ClassificationModel extends Model {
  _preprocess(image) {
    ...
    const rgb_image = tf.image.cropAndResize(
      image.expandDims().toFloat(),
      [[top / h, left / w, (top + size) / h, (left + size) / w]],
      [0],
      [this.input_size, this.input_size]
    )
